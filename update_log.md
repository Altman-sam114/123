...
- 2026-07-07：v4.6 国势面板“终局名分”只读小片落地：`BattleObjectivePanelView` 在“天下棋眼”和“要冲缺口”之间新增“终局名分”竞逐条，按清、顺、西、明等既有胜负线展示各方已控城关、尚缺门槛、判定时点、名分理由和最高分缺口，让国势面板更直观呈现诸方离称制、入关、据中原或保社稷还有多远。该片只影响 SwiftUI 展示，只读既有 `BattleObjectiveSummary.tracks`、`Track`、`Target`、`Faction` 与 `VictoryReason`，不新增 `GameState`、胜负 schema、目标状态、任务状态、塘报、AI、朝廷、经济或规则字段，不新增按钮，不触发目标定位，不写塘报，不提交命令，不改变 `BattleObjectiveSummary` 生成、`VictoryRules`、`Command`、`WarCommandExecutor`、`CommandValidator`、`RuleEngine` 或任何 hex/region/theater/front/deploy 权威。
- 2026-07-07：v4.6 明末军情抽屉“眉批三签”只读小片落地：`RootGameView` 在军情抽屉 tab rail 下新增“军情眉批”三签，按当前面板、最急五线和本旬任务只读提示玩家本次打开抽屉应先看诸方、要冲、朝议、府库、军令、州府、督师、塘报或军机，并把 `BattleObjectiveSummary` 的五线压力与任务摘要前置到军情入口。该片只影响 SwiftUI 展示，不新增 `GameState`、面板 schema、选择状态、目标状态、任务状态、塘报、AI、朝廷、经济或规则字段，不触发目标定位，不写塘报，不提交命令，不改变 `selectedCompactPanel` 行为、子面板入参/回调、`BattleObjectiveSummary`、`Command`、`WarCommandExecutor`、`CommandValidator`、`RuleEngine` 或任何 hex/region/theater/front/deploy 权威。
- 2026-07-07：v4.6 明末舆图“点验候报”只读小片落地：`RootGameView` 在未选中 hex / 州府时为 `MingMapPointInspectionStrip` 增加极薄“候点验”状态条，用城关、粮台、军牌、四线四个小签只读展示要冲分布、粮道显隐、选中军牌摘要和最急五线，让地图第一屏未选中时也保留明末舆图判读语境。该片只影响 SwiftUI 展示，不新增 `GameState`、地图 schema、选择状态、目标状态、任务状态、塘报、AI、朝廷、经济或规则字段，不触发目标定位，不写塘报，不提交命令，不改变 `AppContainer.handleBoardTap`、`selectedRegionInspectorState`、`BattleObjectiveSummary`、`SupplyRules`、`Command`、`WarCommandExecutor`、`CommandValidator`、`RuleEngine` 或任何 hex/region/theater/front/deploy 权威。
- 2026-07-07：v4.6 明末舆图“图层令轨”只读小片落地：`RootGameView` 将顶部地图“图层” segmented picker 改为横向滚动舆图图层按钮轨，按“舆图 / 州府 / 初划 / 战局 / 前线 / 布防”展示系统图标、当前图层名和读图短题，让地图、州府、方面、动态战局、前线和布防切换更像明末军机舆图控件，并保持 44pt 触控高度。该片只影响 SwiftUI 展示和 `MapDisplayLayer` 选择控件外观，仍调用既有 `AppContainer.setMapDisplayLayer`，不新增 `GameState`、地图 schema、图层枚举、按钮行为、胜负、任务、目标、塘报、AI、朝廷、经济或规则字段，不触发目标定位，不写塘报，不提交命令，不改变 `MapDisplayAdapter`、`BoardRenderState`、`Command`、`WarCommandExecutor`、`CommandValidator`、`RuleEngine` 或任何 hex/region/theater/front/deploy 权威。
- 2026-07-07：v4.6 明末军情面板“天下前置导航”只读小片落地：`RootGameView` 将军情抽屉默认入口从“军队”改为“天下”，并把原 9 项 segmented picker 改为横向滚动军情 tab rail，按“天下 / 国势 / 朝廷 / 钱粮 / 军队 / 州府 / 将领 / 塘报 / 军机”前置中华世界局势、胜负要冲、朝议和府库入口；每个 tab 使用图标、短副题、当前说明和 44pt 触控按钮，子面板内容与回调保持原样。该片只影响 SwiftUI 展示和 `selectedCompactPanel` 默认值，不新增 `GameState`、面板 schema、按钮行为、胜负、任务、目标、塘报、AI、朝廷、经济或规则字段，不触发目标定位，不写塘报，不提交命令，不改变 `Command`、`WarCommandExecutor`、`CommandValidator`、`RuleEngine` 或任何 hex/region/theater/front/deploy 权威。
- 2026-07-07：v4.6 明末舆图“城关印框”只读小片落地：`HexNode` 与 `TerrainStyle` 为城池、关隘/堡寨和粮台 hex 增加低噪声印框描边，按城、关、粮台分别使用金、朱砂和青绿墨线，`RootGameView` 在舆图图例补充“城关印框 / 要冲 / 粮台”，让城关粮台在第一视野更像明末舆图要冲。该片只影响 SpriteKit/SwiftUI 展示，只读既有 `HexDisplayState.cityName`、`fortressName`、`controller` 与 `supplySourceFaction`，不新增 `GameState`、地图 schema、城防、补给、胜负、任务、目标、塘报、AI、朝廷、经济或规则字段，不新增按钮，不触发目标定位，不写塘报，不提交命令，不改变 `Command`、`WarCommandExecutor`、`CommandValidator`、`RuleEngine` 或任何 hex/region/theater/front/deploy 权威。
- 2026-07-07：v4.6 明末舆图“驿道墨线”只读小片落地：`BoardScene` 将既有 road 线升级为暗墨底线、浅金主线和驿站小印的驿道纹理，`RootGameView` 在舆图图例补充“驿道 / 行军粮线”，让地图道路更像明末官道、粮线和行军通路。该片只影响 SpriteKit/SwiftUI 展示，只读既有 `HexTile.hasRoad` 与 `MapState.tiles`，不新增 `GameState`、地图 schema、道路、补给、胜负、任务、目标、塘报、AI、朝廷、经济或规则字段，不新增按钮，不触发目标定位，不写塘报，不提交命令，不改变 `SupplyRules`、`Command`、`WarCommandExecutor`、`CommandValidator`、`RuleEngine` 或任何 hex/region/theater/front/deploy 权威。
- 2026-07-07：v4.8 明末朝报令条“战局续读小牌”只读小片落地：`NewGameButton` 将 HUD 里的“战局”菜单入口从普通按钮升级为小型状态牌，读取既有 `SavedGameInfo` 展示可续读势力与回合，无存档时显示新开状态，让发布候选自动保存/续战闭环在第一屏更可见。该片只影响 SwiftUI 展示，不新增 `GameState`、`SavedGameSnapshot`、存档 schema、胜负、任务、目标、塘报、AI、朝廷、经济或规则字段，不新增按钮，不改变继续战局或新开战局行为，不写塘报，不提交命令，不改变 `SavedGameStore`、`AppContainer`、`Command`、`WarCommandExecutor`、`CommandValidator`、`RuleEngine` 或任何 hex/region/theater/front/deploy 权威。
- 2026-07-07：v4.6 明末塘报战记“五线急报”只读小片落地：`EventLogView` 在塘报战记顶部新增只读五线急报条，`RootGameView` 向塘报面板注入既有 `BattleObjectiveSummary.from(state:)`，展示天下、政策、经济、科技、军事五线状态、压力和急务数，让复盘入口也能扫读中华世界局势与军政钱粮火器压力。该片只影响 SwiftUI 展示，不新增 `GameState`、`GameLogEntry`、胜负、任务、目标、塘报、AI、朝廷、经济或规则字段，不新增按钮，不触发目标定位，不写塘报，不提交命令，不改变 `BattleObjectiveSummary`、`Command`、`WarCommandExecutor`、`CommandValidator`、`RuleEngine` 或任何 hex/region/theater/front/deploy 权威。
- 2026-07-07：v4.6 明末军情入口“天下小印”补齐：`RootGameView` 将左下角军情抽屉入口从普通文字按钮升级为只读小型军情徽签，显示展开/收起、当前面板、明末要冲领先方旗号、要冲分和最急五线/急务数，让地图第一视野的主入口持续提示中华世界局势、政策、经济、科技、军事压力。该片只影响 SwiftUI 展示，不新增 `GameState`、胜负、任务、目标、塘报、AI、朝廷、经济或规则字段，不新增按钮行为，不触发目标定位，不写塘报，不提交命令，不改变 `BattleObjectiveSummary`、`Command`、`WarCommandExecutor`、`CommandValidator`、`RuleEngine` 或任何 hex/region/theater/front/deploy 权威。
- 2026-07-07：v4.7 明末舆图“要冲常驻小印”补齐：`BoardScene` 在明末剧本渲染 hex 时只读 `MapState.objectives` 建立 objective 坐标索引，并把非当前聚焦目标且当前可见的 objective 传给 `HexNode`；`HexNode` 在 objective hex 右下角绘制低噪声“要/要冲分”小印，颜色从当前控制方派生，避免未见格泄露信息，也让地图常态就能扫读中华世界局势要冲筹码。该片只影响 SpriteKit 展示，不新增 `GameState`、`MapState`、`Objective`、胜负、任务、塘报、AI、朝廷或经济字段，不触发目标定位，不写塘报，不提交命令，不改变 `BattleObjectiveSummary`、`VictoryRules`、`Command`、`WarCommandExecutor`、`CommandValidator`、`RuleEngine` 或任何 hex/region/theater/front/deploy 权威。

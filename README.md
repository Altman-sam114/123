...
- v4.6 地图标识首片继续 polish 默认主地图：`BaseTerrain.displayName` 改为明末中文地形名，`HexNode` 为城池、关隘/堡寨和补给源增加“城 / 关 / 粮”舆图 badge，并把旧 `FORT` 与 `SUP A/G` 主地图标记改为“关隘”“粮台”；当前又给平原、林地、山地、丘陵、城池和关隘增加低透明“田 / 林 / 山 / 丘 / 城 / 关”地貌底纹，让 hex 底图更像明末舆图。该变化只影响 SpriteKit 展示，不改补给、占领或战区规则。

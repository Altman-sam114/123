...
- v4.6 粮道线路首片复用 `SupplyRules` 的 hex 级补给路径，在默认 hex 图层为玩家势力有有效补给线的军队绘制粮道虚线；路线位于战争迷雾下方、军牌下方，只作可视化，不新增粮道状态或改变补给判定。

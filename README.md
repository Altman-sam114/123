...
- v4.6 命令交互回执继续 polish 军令代入感：`AppContainer` 在提交 `Command` 前按当前 `GameState` 生成 UI-only 中文命令摘要，把移动、攻击、固守、退守许可、补给、募兵筹粮、朝廷项目和结束阶段转成军令口径；普通命令成功/失败、将令提交/诊断、目标定位、单位点选、AI 回合摘要和 fallback mock commander 名称都改为中文，`CommandValidationError` 在显示层转为中文驳回原因。该片只影响 `lastCommandMessage` 与 `interactionLog` 展示，不改变 `Command.displayName`、命令校验、执行结果、记录 ID、`Command`、`CommandValidator`、`WarCommandExecutor`、`RuleEngine` 或任何规则权威。

...
- v4.6 州府牌首片继续 polish 州府界面：`RegionInspectorView` 从字段列表升级为州府牌，读取 `RegionInspectorState` 展示城关粮坊、地方治理、钱粮城防、控制方旗号、原属章、方面/防区/目标、友敌军和当前格旗号；当前进一步把州府和当前格的方面/防区 raw id、裸坐标转成“关宁防线”“畿辅防区”“舆图格”等可读文案；该片只影响 SwiftUI 展示，不改变 hex 控制、region 聚合、经济结算、动态战区、前线或命令规则。

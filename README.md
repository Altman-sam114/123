...
> v4.7 最新增量：`DataLoader` 会把 `ScenarioDefinition.victoryConditions` 写入 `GameState.victoryConditions`，`BattleObjectiveSummary` 优先从剧本条件编译明末胜负线和 objective points 领先方，缺失时才回退旧内置目标；`VictoryRules` 与新增“目标”信息面板共用该摘要，玩家现在可在局内查看清破关入京、大顺据中原秦陕、大西据湖广粮区和明廷守京师关口各差哪些城关。

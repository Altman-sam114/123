...
地图编辑器/JSON 数据
  ...
  -> v4.7 明末胜负链首片把 ScenarioDefinition.victoryConditions 写入 GameState，并让清破关入京、大顺据中原秦陕、大西据湖广粮区、明廷守住京师关口等条件进入 BattleObjectiveSummary / VictoryRules，在“目标”面板显示进度、只读战役提示、本旬任务链和阶段战局链，回合末把提示和急务/主线任务入塘报；目标 chip 和任务按钮可只读定位对应 hex / 州府；目标 hex 换手会写塘报

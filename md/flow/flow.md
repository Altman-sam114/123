...
- v4.6/v4.7 舆图天下急势首片中，`RootGameView` 顶部舆图控件在明末剧本下复用 `BattleObjectiveSummary.from(state:)` 显示只读“天下急势”条，包含 objective points 领先方、要冲分布、本旬先手、急务/主线任务数，以及天下、政策、经济、科技、军事五线压力 chip；“要冲分布”读取 `scoreRows` 横向显示各势力要冲分和控制要冲数，并标出领先方；“本旬先手”读取 `tasks` 与 `tracks.targets`，展示最高优先级任务、目标城关、当前控制方和要冲分；当前“舆图点验”只读读取 `selectedRegionInspectorState`、`BattleObjectiveSummary.lineBriefs`、选中 `Division` 和粮道开关状态，在点选 hex、部队或目标后展示选中格位/州府、控制方旗号、动态方面、防区、友敌军、要冲、前线压力、最急五线、军粮状态、路线显隐、兵力和可调/已行，帮助玩家在地图第一视野联读战局归属、中华世界局势、政策/经济/科技/军事压力、粮道和当前部队兵势。它只把既有胜负线、任务摘要、选中状态、粮道显示状态和部队摘要前置到地图第一视野，不调用 `AppContainer.focusObjective(_:)`，不写塘报，不新增持久状态，不改变胜负、朝廷、AI、命令、补给判定、hex/region/theater/front/deploy 或规则权威。

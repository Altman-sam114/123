...
MapEditor / JSON 数据
  ...
  -> BoardRenderState.showsSupplyRoutes / PlayerCommandState 军令计划线 / MapDisplayLayer 舆图图例 / UnitTooltipView 舆图军牌浮签与军位 / UI overlay / 塘报战记 / WarDirectiveRecord
```
...
- v4.6 舆图军牌浮签首片中，`UnitTooltipView` 从旧摘要浮窗升级为明末舆图军牌浮签，读取选中地图部队的 `Division` 展示势力旗号、兵力条、粮草/行动/退守状态、攻守行程察指标和兵种组件 chip；当前增强把裸坐标和退守短码改为“舆图格”“退守中”“余 N 旬”，并读取 `UnitInspectorStrategicState` 只读展示“军位”区，把动态方面、防区和前线/纵深/驻防部署角色放进地图浮签；它只服务地图选中部队的快速扫读和明末代入感，不改 `Division`、战斗/补给/部署规则、`Command`、`WarCommandExecutor` 或 `RuleEngine`。
...
- `UnitTooltipView` 以明末舆图军牌浮签展示选中地图部队，读取同一 `Division` 的势力旗号、兵力、粮草/行动/退守、攻守行程察和兵种组件 chip，并从 `UnitInspectorStrategicState` 读取动态方面、防区和部署角色，显示为只读“军位”chip；它不触发 `Command`，不改变 `Division`、`SupplyRules`、`CombatRules`、`WarDeploymentState` 或任何规则权威。

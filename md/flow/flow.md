# WWIIHexV0 核心流程文档（明末迁移 v4.7 剧本胜利条件数据驱动、明末胜负链、战役目标面板/天下棋眼、舆图要冲分布、廷议要冲、府库四线牵引、天下五线态势、朝廷五线态势、军机五线态势、本旬任务链/任务塘报、开封围城压力提示、AI doctrine、军机势力军略展示、目标定位舆图反馈、目标换手塘报与阶段战局链首片，v4.6 UI、朝廷项目、朝廷项目行动状态提示、整训团练地方驻防、朝议总纲、朝议批票、朝议争点、朝报令条、军令牌、将印军令/将领名帖、军机复盘牌/军机底稿/命令短令、塘报战记/急务战役分类/默认中文塘报、军情牌/军令战备/军械火力、舆图军牌浮签、地图军牌战备小签、州府牌归属旗号、府库牌收支急报/军饷民心/生产状态提示、天下急势/天下牵引、舆图天下急势、地图标识、粮道线路/开关、军令计划线、势力旗号、舆图图例/舆图判读与四线项目分组首片）
...
MapEditor / JSON 数据
  ...
  -> DiplomacyState 关系判断
  -> DiplomacyPanelView 天下急势 / 天下牵引只读摘要
  -> Hex controller / Division coord
  -> Region 聚合
  -> EconomyState 收入 / 生产 / 补员
  -> EconomyPanelView 府库四线牵引只读展示
  ...
  -> BattleObjectivePanelView 天下棋眼 / 目标 chip 只读显示控制方旗号并定位 selectedHex / selectedRegionId / focusedObjectiveId
...
- v4.6 部队军情牌首片中，`UnitInspectorView` 从字段列表升级为只读军情牌，展示军牌字、兵力条、粮草/退守/行动、攻守行程察指标、兵种编成条和驻防归属；后续增强新增“军令战备”只读摘要，基于 `Division.canAct`、`supplyState`、`strengthRatio`、退守和部队编成派生可调/已行/断粮、粮道、战力和用兵定位；当前新增“军械火力”只读区，基于 `Division.components`、火器/炮队/攻城器械比例、`effectiveStats.range`、`hasFireSupport`、`isSiegeCapable`、粮草和兵力状态派生火力姿态与军械占比，帮助部队牌解释科技/火器/攻城价值；当前增强把驻防归属中的州府、方面、防区、前线和格位 id 通过 `MingMapLabelFormat` 转成明末可读文案，并把“玩家/只读”改为“本方可调/他方军情”；该片只读 `Division` 与 `UnitInspectorStrategicState`，不改变战斗、补给、部署或命令规则。
...
- v4.6 军令牌首片中，`CommandPanelView` 从简单标题和按钮列升级为军令牌，展示当前势力/阶段、选中军情、兵力、粮草、退守、行动、固守/退守/补给处置和军令回执；当前增强新增只读“舆图军令”提示，按观战、未选军、敌军、非本方阶段、已行动和粮草状态说明调动/攻击要在舆图点目标格，固守、准退和补给在军令牌内批令，并把坐标统一改为 `MingMapLabelFormat.coordinate` 的“舆图格”口径；`AppContainer` 在提交 `Command` 前按当前 `GameState` 生成 UI-only 中文命令摘要，并把普通命令、玩家将令、单位点选、目标定位和 AI 回合回执写成明末中文口径；固守、准许退守、就地补给和结束回合仍只调用 `RootGameView` 注入的原有回调，不直接修改 `GameState`，不改变 `Command.displayName`、`CommandValidator`、`WarCommandExecutor` 或 `RuleEngine`。
...
- v4.6 军机复盘牌首片中，`AgentPanelView` 从 LabeledContent 调试列表升级为只读军机复盘牌，展示决策摘要、最高意志、军机五线态势、战区指令、势力军略、命令回执、异常塘报和军机底稿；当前增强把主事、来源、主上、重心、目标、指向等 id 通过共享 `MingMapLabelFormat` 只读转成明末可读名称，并在命令结果显示边界把稳定 `Command.displayName` 前缀映射为调动、攻击、固守、准退、补给整备、营造筹备和结束阶段；最新增强继续把 `AgentDecisionRecord.errors`、`CommandResultSummary.errors` 中的已知 `CommandValidationError.rawValue` 转成 `mingDisplayText`，把 `Mapping failed.`、`No AI faction was active.`、未知 provider 和未收录 doctrine skill 转成军机案卷口径，未知值仍保留可读兜底便于审计；该片只读 `AgentDecisionRecord`、`RulerDecisionRecord`、`WarDirectiveRecord`、`CommandResultSummary`、`CampaignAISummary` 与 `ZoneCommanderDoctrine.profile(for:)`，不改变 `Command.displayName` 本体、Codable 记录、AI prefix 判定、`MarshalAgent`、`RulerAgent`、`WarCommandExecutor`、`Command` 或 `RuleEngine`。
...
- v4.6/v4.7 府库牌首片中，`EconomyPanelView` 从表格/按钮列表升级为只读府库牌加生产入口，展示民力、银两、粮草库存、本回合入账、军粮维护、补员消耗、收支急报、净民力/银两/粮草、府库四线牵引、内政钱粮治理摘要、军饷民心只读态势、募兵筹粮和营造队列；“府库四线牵引”只读 `FactionEconomyLedger` 和当前势力 `CourtStrategySummary`，展示政策、经济、科技、军事压力、库存、营造队列、主议和备议，帮助府库面板联读朝廷取舍、钱粮余势、火器支点和接战压力；“内政钱粮”只读 `GovernanceAISummary.from(faction:map:)`，显示州府、不稳、民变、行政和最低行政州府，“军饷民心”只读 `FactionEconomyLedger`、当前势力未毁部队补给状态和同一治理摘要，派生军伍、缺粮、军饷余势与民心综合，不新增军饷、士气、民心或灾荒规则；募兵筹粮行只读 `FactionEconomyLedger.stockpile`、`ProductionKind.cost`、观战状态和当前 phase，显示“可开工 / 尚缺民力、银两、粮草 / 待本方 / 观战”，生产按钮仍只走 `Command.queueProduction -> CommandValidator -> EconomyRules`，不直接改经济账本。
- v4.6 天下急势首片中，`DiplomacyPanelView` 从 `DiplomacyState` 和只读 `CourtStrategySummary` 派生顶部“天下急势”、势力战意条、主要对手和政策/经济/科技/军事四线压力；当前增强让朝议/军议主事和重心、国家/阵营 fallback 走 `MingMapLabelFormat`，避免 `ruler_*`、`theater_*`、`ming_court` 等 id 直出；最新增强在“天下概览”后新增只读“天下牵引”区，复用 `DiplomacyState`、当前 active faction 的 `CourtStrategySummary` 和最近 `RulerDecisionRecord.diplomacySummary` 展示战和格局、朝议牵引、四线压力和御前奏报；该片只影响 SwiftUI 展示，不改变外交关系、朝廷项目或规则执行。
...
- `CourtStrategySummary`、`BattleObjectiveSummary` 与 `CampaignAISummary` 都是只读派生摘要，不直接改 `GameState`；`BattleObjectiveSummary.Cue` 用于目标面板的历史/教程提示和回合末塘报入册，不执行事件效果；开封围城压力 cue 只读 objective 控制方，不新增真实 siege state、城防损耗或事件执行器；`BattleObjectiveSummary.CampaignTask` 只把当前胜负线压力、回合、开封压力和明廷火器支点翻译成目标面板的本旬任务链，并可由 `appendBattleTaskEvents` 将急务/主线任务写入塘报，不保存持久任务进度，不执行政策/经济/科技/军事效果；`BattleObjectiveSummary.CampaignLineBrief` 只把既有阶段链和任务链聚合为天下、政策、经济、科技、军事五线扫读摘要，不新增政策或科技状态；`RootGameView` 的“天下急势/要冲分布”舆图条、`CourtPanelView` 的“廷议要冲”和“天下五线态势”都只读取该摘要并在非明末剧本隐藏，不触发定位、不触发项目、不写塘报、不改变 `CourtStrategySummary` 的四线排序；`CampaignAISummary` 只把这些五线摘要转成 Agent/元帅/军机复盘牌可读文本和结构化字段，不下发命令；`AgentPanelView` 的“军机五线”和“势力军略”只读展示该摘要和 `ZoneCommanderDoctrine.profile(for:)` 的 title/style/skills/tactic 偏向，不改变 prompt、schema、doctrine、AI 偏置或命令执行；`ZoneCommanderDoctrine` 只改变默认 commander、MockAI 与模拟元帅 JSON 上游的风格、技能标签和 tactic 偏置，最终仍必须产出 `TheaterDirective` 或 `ZoneDirective` 并进入统一执行链；`CourtStrategySummary` 只把战役线、地方中立/非核心州府、低粮田水利和治理压力加权进主议/备议排序与理由，不执行朝廷项目；目标换手塘报只记录已经发生的 objective hex 控制变化，不替代 `VictoryRules` 或 `BattleObjectiveSummary`；`BattleObjectivePanelView` 的目标城关控制方旗号只读展示 `BattleObjectiveSummary.Target.controller`，不驱动胜负、目标换手、定位或地图状态；`CourtPanelView` 的朝议批票、廷议要冲、朝议争点和 `CourtProjectDomain` 只服务票拟解释、要冲归属扫读、四线展示、争点表达和项目分组，可执行朝廷项目必须走 `Command.enactCourtProject -> CommandValidator -> CommandExecutor -> EconomyRules`。`HUDView` 的朝报令条、`BattleObjectivePanelView` 的目标面板与目标定位、`CommandPanelView` 的军令牌、`GeneralCommandPanelView` 的将印军令、`GeneralProfileView` 的将领名帖、`AgentPanelView` 的军机复盘牌、`EventLogView` 的塘报战记和急务/战役分类、`UnitInspectorView` 的军情牌、`UnitTooltipView` 的舆图军牌浮签、`RegionInspectorView` 的州府牌、`EconomyPanelView` 的府库牌、`DiplomacyPanelView` 的天下急势、`RootGameView` 的舆图天下急势条和要冲分布、`BoardScene.drawFocusedObjective` 的目标定位令牌与胜负线城关连线、`BoardScene.drawPlannedOperations` 的军令计划线、`UnitNode` / `MingFactionFlagBadge` 的势力旗号、`AppContainer.showsSupplyRoutes` 和 `MapDisplayLayer` 图例元数据只控制 UI 展示。`RulerAgent` 仍不是默认主链路。
- `EconomyPanelView` 的“府库四线牵引”和 `DiplomacyPanelView` 的“天下牵引”同样只属于 UI 展示边界：前者从既有府库账本和朝议摘要派生可读文本，后者从既有外交关系、朝议摘要和最近朱批外交摘要派生可读文本；二者都不新增经济、朝廷、外交或科技字段，不写塘报，不触发生产、目标定位、朝廷项目、AI 决策或任何命令/规则执行。
...
`EconomyPanelView` 当前以府库牌展示 `FactionEconomyLedger`，把库存、入账、军粮维护、补员消耗、收支急报、净民力/银两/粮草、军饷民心、募兵筹粮和营造队列放在同一钱粮界面；军饷民心只读 ledger、当前势力部队补给状态和治理摘要派生压力，不新增真实军饷/士气/民心字段；募兵筹粮行只读库存与成本差额显示可开工、尚缺资源、待本方或观战；它只读 ledger 并提交生产命令，不直接写 `EconomyState`。
...
- `CommandPanelView` 以军令牌展示当前 active faction、phase、选中军情、兵力、粮草、退守、行动、舆图军令提示、固守/退守/补给处置和最近军令回执；坐标统一显示为“舆图格”，提示文案只按现有状态说明移动/攻击在地图点目标格、固守/退守/补给在面板批令；它只负责 SwiftUI 展示，固守、准许退守、就地补给和结束回合仍通过 `RootGameView` 注入的 `AppContainer` 回调进入原命令链。
...
- `AgentPanelView` 以军机复盘牌展示 AI 与军机记录，读取 `AgentDecisionRecord` 的主事、来源、意图、局势摘要、legacy command results、错误和 raw JSON；读取 `RulerDecisionRecord` 的最高意志、姿态、重心、目标、攻势阈、留营、天下判断和朱批理由；读取 `WarDirectiveRecord` 的战区、势力、军机、督师、战术、目标、指向、命令成功/驳回数量和 diagnostics；读取 `CampaignAISummary.from(state:)` 的五线压力、告急状态和当旬急务，作为“军机五线”只读复盘区；读取 `ZoneCommanderDoctrine.profile(for: directive.faction)` 的军略名、指挥风格、技能标签和势力战术偏向，作为每条战区指令下的“势力军略”解释。主事、来源、主上、重心、目标和指向的底层 id、provider、validation rawValue、mapping fallback 和 doctrine skill 只在 SwiftUI 中转成明末可读名称，未知值仍回退可读案卷文本；原始 raw JSON 仍作为“军机底稿”保留 text selection 审计能力。它只负责 SwiftUI 展示，不写入 `GameState`，也不改变 `MarshalAgent`、`RulerAgent`、`ZoneCommanderDoctrine`、`WarCommandExecutor`、`Command`、`CommandValidator` 或 `RuleEngine`。

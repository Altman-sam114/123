...
- v4.6/v4.7 军令牌首片中，`CommandPanelView` 从简单标题和按钮列升级为军令牌，展示当前势力/阶段、选中军情、兵力、粮草、退守、行动、固守/退守/补给处置和军令回执；后续新增只读“舆图军令”提示，按观战、未选军、敌军、非本方阶段、已行动和粮草状态说明调动/攻击要在舆图点目标格，固守、准退和补给在军令牌内批令，并把坐标统一改为 `MingMapLabelFormat.coordinate` 的“舆图格”口径；当前新增只读“要冲军令”区，由 `RootGameView` 注入 `BattleObjectiveSummary.from(state:)`，读取本旬任务、五线态势、目标 track 和选中 `Division`，在军令牌中显示本旬急务、目标落点与本军兵势；`AppContainer` 在提交 `Command` 前按当前 `GameState` 生成 UI-only 中文命令摘要，并把普通命令、玩家将令、单位点选、目标定位和 AI 回合回执写成明末中文口径；固守、准许退守、就地补给和结束回合仍只调用 `RootGameView` 注入的原有回调，不直接修改 `GameState`，不改变 `BattleObjectiveSummary`、`VictoryRules`、`Command.displayName`、`CommandValidator`、`WarCommandExecutor` 或 `RuleEngine`。
...
- `CourtStrategySummary`、`BattleObjectiveSummary` 与 `CampaignAISummary` 都是只读派生摘要，不直接改 `GameState`；`BattleObjectiveSummary.Cue` 用于目标面板的历史/教程提示和回合末塘报入册，不执行事件效果；开封围城压力 cue 只读 objective 控制方，不新增真实 siege state、城防损耗或事件执行器；`BattleObjectiveSummary.CampaignTask` 只把当前胜负线压力、回合、开封压力和明廷火器支点翻译成目标面板的本旬任务链，并可由 `appendBattleTaskEvents` 将急务/主线任务写入塘报，不保存持久任务进度，不执行政策/经济/科技/军事效果；`BattleObjectiveSummary.CampaignLineBrief` 只把既有阶段链和任务链聚合为天下、政策、经济、科技、军事五线扫读摘要，不新增政策或科技状态；`RootGameView` 的“天下急势/要冲分布”舆图条、`CourtPanelView` 的“廷议要冲”和“天下五线态势”、`CommandPanelView` 的“要冲军令”都只读取该摘要并在非明末剧本隐藏或不展示，不触发定位、不触发项目、不写塘报、不提交命令、不改变 `CourtStrategySummary` 的四线排序；`CampaignAISummary` 只把这些五线摘要转成 Agent/元帅/军机复盘牌可读文本和结构化字段，不下发命令；`AgentPanelView` 的“军机五线”和“势力军略”只读展示该摘要和 `ZoneCommanderDoctrine.profile(for:)` 的 title/style/skills/tactic 偏向，不改变 prompt、schema、doctrine、AI 偏置或命令执行；`ZoneCommanderDoctrine` 只改变默认 commander、MockAI 与模拟元帅 JSON 上游的风格、技能标签和 tactic 偏置，最终仍必须产出 `TheaterDirective` 或 `ZoneDirective` 并进入统一执行链；`CourtStrategySummary` 只把战役线、地方中立/非核心州府、低粮田水利和治理压力加权进主议/备议排序与理由，不执行朝廷项目；目标换手塘报只记录已经发生的 objective hex 控制变化，不替代 `VictoryRules` 或 `BattleObjectiveSummary`；`BattleObjectivePanelView` 的目标城关控制方旗号和要冲缺口只读展示 `BattleObjectiveSummary.Target.controller`、`Target.isControlled` 和目标分值，不驱动胜负、目标换手、任务状态、塘报、AI、定位或地图状态；`CourtPanelView` 的朝议批票、廷议要冲、朝议争点和 `CourtProjectDomain` 只服务票拟解释、要冲归属扫读、四线展示、争点表达和项目分组，可执行朝廷项目必须走 `Command.enactCourtProject -> CommandValidator -> CommandExecutor -> EconomyRules`。`HUDView` 的朝报令条、`BattleObjectivePanelView` 的目标面板与目标定位、`CommandPanelView` 的军令牌/舆图军令/要冲军令、`GeneralCommandPanelView` 的将印军令、`GeneralProfileView` 的将领名帖、`AgentPanelView` 的军机复盘牌、`EventLogView` 的塘报战记和急务/战役分类、`UnitInspectorView` 的军情牌、`UnitTooltipView` 的舆图军牌浮签、`RegionInspectorView` 的州府牌、`EconomyPanelView` 的府库牌、`DiplomacyPanelView` 的天下急势、`RootGameView` 的舆图天下急势条和要冲分布、`BoardScene.drawFocusedObjective` 的目标定位令牌与胜负线城关连线、`BoardScene.drawPlannedOperations` 的军令计划线、`UnitNode` / `MingFactionFlagBadge` 的势力旗号、`AppContainer.showsSupplyRoutes` 和 `MapDisplayLayer` 图例元数据只控制 UI 展示。`RulerAgent` 仍不是默认主链路。
- `EconomyPanelView` 的“府库四线牵引”“经世策眼”和 `DiplomacyPanelView` 的“天下牵引”同样只属于 UI 展示边界：府库四线牵引从既有府库账本和朝议摘要派生可读文本，经世策眼从既有战役目标摘要、府库账本和朝议摘要派生可读文本，天下牵引从既有外交关系、朝议摘要和最近朱批外交摘要派生可读文本；它们都不新增经济、朝廷、外交、胜负或科技字段，不写塘报，不触发生产、目标定位、朝廷项目、AI 决策或任何命令/规则执行。
...
- `CommandPanelView` 以军令牌展示当前 active faction、phase、选中军情、兵力、粮草、退守、行动、舆图军令提示、要冲军令、固守/退守/补给处置和最近军令回执；坐标统一显示为“舆图格”，舆图军令只按现有状态说明移动/攻击在地图点目标格、固守/退守/补给在面板批令；要冲军令只读取 `BattleObjectiveSummary` 与选中 `Division`，显示本旬急务、目标落点和兵势，不新增按钮、不定位目标、不提交命令；它只负责 SwiftUI 展示，固守、准许退守、就地补给和结束回合仍通过 `RootGameView` 注入的 `AppContainer` 回调进入原命令链。

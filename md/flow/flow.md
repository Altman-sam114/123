# WWIIHexV0 核心流程文档（明末迁移 v4.7 剧本胜利条件数据驱动、明末胜负链、战役目标面板/国势四策/天下棋眼/要冲缺口、舆图要冲分布/本旬先手、廷议要冲、府库四线牵引/经世策眼/民食灾荒、要冲军令/督师要冲、天下五线态势、朝廷五线态势、军机五线态势、本旬任务链/任务塘报、开封围城压力提示、AI doctrine、军机势力军略展示/诸势军略总览、军机/MockAI摘要中文化、目标定位舆图反馈、目标换手塘报与阶段战局链首片，v4.6 UI、军情/国势入口文案与阶段显示去 Legacy 前缀、朝廷项目、朝廷项目行动状态提示、整训团练地方驻防、朝议总纲、朝议批票、朝议争点、朝报令条、军令牌、将印军令/帷幄四线/将领名帖、军机复盘牌/军机底稿/命令短令、塘报战记/急务战役分类/默认中文塘报、军情牌/军令战备/军械火力、舆图军牌浮签、地图军牌战备小签、州府牌归属旗号、府库牌收支急报/军饷民心/生产状态提示、天下急势/天下牵引、舆图天下急势、地图标识/地貌底纹、粮道线路/开关、军令计划线、势力旗号、舆图图例/舆图判读与四线项目分组首片）
...
MapEditor / JSON 数据
  ...
  -> EconomyPanelView 府库四线牵引 / 经世策眼 / 民食灾荒只读展示
  ...
  -> RootGameView / MingMapSituationStrip 本旬先手只读展示最高优先级任务 / 目标城关 / 控制方 / 要冲分；MingMapPointInspectionStrip 未选中时显示只读候点验空态，选中后读取 selectedRegionInspectorState
...
- v4.6 州府牌首片中，`RegionInspectorView` 从字段列表升级为只读州府牌，展示州府主值、本州入局、州府四线牵引、城关粮坊、地方治理、钱粮城防、控制方旗号、原属章、方面/防区/目标、友敌军和当前格旗号；当前增强把州府和当前格的方面/防区 raw id、裸坐标转成“关宁防线”“畿辅防区”“舆图格”等可读文案；“本州入局”从现有目标、前线、治理、粮台、工坊、驿道和友敌军派生要冲入局、接敌入局、地方入局、经略入局或后方入局总批，并用天下、政粮、军械三枚 chip 联读中华世界局势、政策、经济、科技和军事；州府四线牵引只从现有治理、钱粮、工坊驿道、目标、友敌军和前线压力派生政策/经济/科技/军事四格；该片只读 `RegionInspectorState`、`RegionNode` 和 `OccupationState`，不改变 hex 控制、region 聚合、经济结算、前线/部署或命令规则。
- v4.6/v4.7 府库牌首片中，`EconomyPanelView` 从表格/按钮列表升级为只读府库牌加生产入口，展示民力、银两、粮草库存、本回合入账、军粮维护、补员消耗、收支急报、净民力/银两/粮草、府库四线牵引、经世策眼、民食灾荒、内政钱粮治理摘要、军饷民心只读态势、募兵筹粮和营造队列；“府库四线牵引”只读 `FactionEconomyLedger` 和当前势力 `CourtStrategySummary`，展示政策、经济、科技、军事压力、库存、营造队列、主议和备议，帮助府库面板联读朝廷取舍、钱粮余势、火器支点和接战压力；“经世策眼”只读 `BattleObjectiveSummary`、`CourtStrategySummary` 和 `FactionEconomyLedger`，展示 objective points 领先方、最急天下五线、府库粮银余势、本旬主议与备议，帮助府库牌从钱粮视角联读中华世界局势、要冲分、粮道、地方治理和本旬取舍；“民食灾荒”只读同一府库账本、治理摘要和当前势力未毁军伍补给状态，派生库存粮、本旬粮差、民食余势、不稳州府和断粮军伍，用于提示灾荒风险但不执行事件效果；“内政钱粮”只读 `GovernanceAISummary.from(faction:map:)`，显示州府、不稳、民变、行政和最低行政州府，“军饷民心”只读 `FactionEconomyLedger`、当前势力未毁部队补给状态和同一治理摘要，派生军伍、缺粮、军饷余势与民心综合，不新增军饷、士气、民心或灾荒规则；募兵筹粮行只读 `FactionEconomyLedger.stockpile`、`ProductionKind.cost`、观战状态和当前 phase，显示“可开工 / 尚缺民力、银两、粮草 / 待本方 / 观战”，生产按钮仍只走 `Command.queueProduction -> CommandValidator -> EconomyRules`，不直接改经济账本。
...
- v4.6 地图标识首片中，`BaseTerrain.displayName` 已切为明末中文地形名；`HexNode` 用“城 / 关 / 粮”badge 标识城池、关隘/堡寨和粮台，并把旧主地图 `FORT`、`SUP A/G` 标记改为“关隘”“粮台”；当前又由 `TerrainStyle.terrainMotif` 和 `HexNode.addTerrainMotif` 为平原、林地、山地、丘陵、城池、关隘增加低透明“田 / 林 / 山 / 丘 / 城 / 关”地貌底纹；当前城关/粮台 hex 会复用既有 `Faction.bannerGlyph` 与 `TerrainStyle.controllerColor(for:)` 显示控制方旗号，普通 controlled hex 仍保留低噪声圆点。该变化只影响 SpriteKit 展示，只读 `HexDisplayState.controller` 与 `supplySourceFaction`，不改 `HexTile.controller`、地形类型、补给、占领、胜负、战区或经济规则。
...
- v4.6/v4.7 舆图天下急势首片中，`RootGameView` 顶部舆图控件在明末剧本下复用 `BattleObjectiveSummary.from(state:)` 显示只读“天下急势”条，包含 objective points 领先方、要冲分布、本旬先手、急务/主线任务数，以及天下、政策、经济、科技、军事五线压力 chip；“要冲分布”读取 `scoreRows` 横向显示各势力要冲分和控制要冲数，并标出领先方；“本旬先手”读取 `tasks` 与 `tracks.targets`，展示最高优先级任务、目标城关、当前控制方和要冲分；当前“舆图点验”有 empty / selected 两种展示态，empty 只在未选中 hex / 州府时显示“点舆图格”候点验空态，用城关、粮台、军牌和四线小签说明第一屏读图入口，selected 继续只读读取 `selectedRegionInspectorState`、`BattleObjectiveSummary.lineBriefs`、选中 `Division` 和粮道开关状态，在点选 hex、部队或目标后展示选中格位/州府、控制方旗号、动态方面、防区、友敌军、要冲、前线压力、最急五线、军粮状态、路线显隐、兵力和可调/已行，帮助玩家在地图第一视野联读战局归属、中华世界局势、政策/经济/科技/军事压力、粮道和当前部队兵势。它只把既有胜负线、任务摘要、选中状态、粮道显示状态和部队摘要前置到地图第一视野，不调用 `AppContainer.focusObjective(_:)`，不写塘报，不新增持久状态，不改变选择权威、胜负、朝廷、AI、命令、补给判定、hex/region/theater/front/deploy 或规则权威。
...
- v4.7 目标面板国势四策首片中，`BattleObjectivePanelView` 在目标面板 header 后新增只读“国势四策”，从 `BattleObjectiveSummary`、当前势力 `CourtStrategySummary` 和 `FactionEconomyLedger` 合并派生政策、经济、科技、军事四张扫读牌，展示要冲分领先方、朝议主议/备议、府库银粮、火器攻城军、前线压力和最急经济/军事提示；当前四策卡优先读取同名 `BattleObjectiveSummary.CampaignLineBrief`，把政策线、经济线、科技线、军事线的压力、急务和主线说明直接接入第一屏。该片只把胜负线、朝议、府库余势和四线压力放到目标面板第一屏，不新增按钮，不触发目标定位，不写塘报，不执行朝廷项目，不提交命令，不改变 `BattleObjectiveSummary`、`CourtStrategySummary`、`EconomyRules`、`VictoryRules`、`Command`、`WarCommandExecutor`、`CommandValidator`、`RuleEngine` 或任何 hex/region/theater/front/deploy 权威。
- v4.7 明末 AI doctrine 首片中，`ZoneCommanderDoctrine` 按势力生成默认 `ZoneCommanderAgentConfig`：明廷谨慎并偏防京畿/保粮/守城，清方进取并偏旗骑/合围/截援，大顺进取并偏扩粮/破弱城，大西进取并偏流动作战/夺粮，地方中立谨慎自保；`TheaterCommanderPool` fallback、`AppContainer` 空将领 registry fallback、显式 `.zoneDirective` 路径、`MockAICommander` 和 `SimulatedMarshalLLMClient` 都共用同一 doctrine 口径，影响 `BinaryTacticClassifier` 攻守边界、技能标签和默认元帅 JSON 上游 tactic；`ZoneCommanderAgent` 与模拟元帅会按 doctrine 把同态进攻映射为明廷火器压制、清方突骑破阵/合围、大顺破围、大西流动作战；当前增强让明末模拟元帅 `strategicIntent`、`TheaterDirective.rationale`、`TheaterDirectiveCompiler` 摘要和 `MarshalAgent.resolve` fallback 诊断输出军机/督师中文口径，legacy 德/盟路径保留英文诊断；`AgentPanelView` 军机复盘牌也会按 `WarDirectiveRecord.faction` 只读显示 doctrine 的势力军略、指挥风格、技能标签和战术偏向，并在“诸势军略”中只读比较明末全部 launch 势力的 doctrine 摘要；当前增强又把 legacy Agent D 的 `AgentPromptBuilder`、`MockAIClient`、`TurnManager.contextSummary` 和 fallback `GameAgent.sample` 按明末势力改成军机、粮草、城关、州府、火器和天下五线口径，并继续把明末 prompt/MockAI 理由里的 `SupplyState.rawValue` 显示为有粮、缺粮、断粮被围；`RegionId.mingDisplayTitle` 现为 Agent prompt、治理摘要和 MockAI 明末前线理由提供州府名，避免军机案卷直出 `region_*` raw id，legacy 德/盟路径保留阿登/Bastogne 文案。该片不新增 AI 管线，不直接下发底层 `Command`，不改变 `WarCommandExecutor` 或 `RuleEngine` 权威。
...
- `GamePhase.allowsHumanCommands` 是玩家可操作阶段的当前 UI/App 判定入口，兼容 `.humanAction` 与 legacy `.alliedPlayer`；`GamePhase.displayName` 对 legacy `.germanAI` / `.alliedPlayer` 也只显示“军机行动 / 玩家行令”，避免默认明末 UI 直出 `Legacy` 调试前缀。
...
- `CourtStrategySummary`、`BattleObjectiveSummary` 与 `CampaignAISummary` 都是只读派生摘要，不直接改 `GameState`；`BattleObjectiveSummary.Cue` 用于目标面板的历史/教程提示和回合末塘报入册，不执行事件效果；开封围城压力 cue 只读 objective 控制方，不新增真实 siege state、城防损耗或事件执行器；`BattleObjectiveSummary.CampaignTask` 只把当前胜负线压力、回合、开封压力和明廷火器支点翻译成目标面板的本旬任务链，并可由 `appendBattleTaskEvents` 将急务/主线任务写入塘报，不保存持久任务进度，不执行政策/经济/科技/军事效果；`BattleObjectiveSummary.CampaignLineBrief` 只把既有阶段链和任务链聚合为天下、政策、经济、科技、军事五线扫读摘要，不新增政策或科技状态；`HUDView` 的“朝报要冲”、`RootGameView` 的“天下急势/要冲分布/本旬先手/舆图点验四线”舆图条、`CourtPanelView` 的“廷议要冲”和“天下五线态势”、`DiplomacyPanelView` 的“天下棋势”、`CommandPanelView` 的“要冲军令/朝议四线”、`GeneralCommandPanelView` 的“督师要冲”、`UnitInspectorView` 与 `UnitTooltipView` 的“要冲牵引”都只读取该摘要并在非明末剧本隐藏或不展示，不触发定位、不触发项目、不写塘报、不提交命令、不改变 `CourtStrategySummary` 的四线排序；`RootGameView` 的“舆图点验”和 `RegionInspectorView` 的“本州入局”只读取当前 `selectedRegionInspectorState`、`RegionNode`、`OccupationState` 及其中已有的 selected hex / region / dynamic theater / front zone / objective / economy 派生字段；舆图点验额外只读选中 `Division.supplyState`、兵力、行动状态和 `AppContainer.showsSupplyRoutes` 传入的显示开关，不调用 `SupplyRules`、不改变补给判定、不改变选中状态、地图权威或 region schema；`CampaignAISummary` 只把这些五线摘要转成 Agent/元帅/军机复盘牌可读文本和结构化字段，不下发命令；`AgentPanelView` 的“军机五线”“势力军略”和“诸势军略”只读展示该摘要、`Faction.mingLaunchCases` 和 `ZoneCommanderDoctrine.profile(for:)` 的 title/style/skills/tactic 偏向，不改变 prompt、schema、doctrine、AI 偏置或命令执行；`ZoneCommanderDoctrine` 只改变默认 commander、MockAI 与模拟元帅 JSON 上游的风格、技能标签和 tactic 偏置，最终仍必须产出 `TheaterDirective` 或 `ZoneDirective` 并进入统一执行链；`CourtStrategySummary` 只把战役线、地方中立/非核心州府、低粮田水利和治理压力加权进主议/备议排序与理由，不执行朝廷项目；目标换手塘报只记录已经发生的 objective hex 控制变化，不替代 `VictoryRules` 或 `BattleObjectiveSummary`；`BattleObjectivePanelView` 的目标城关控制方旗号和要冲缺口只读展示 `BattleObjectiveSummary.Target.controller`、`Target.isControlled` 和目标分值，不驱动胜负、目标换手、任务状态、塘报、AI、定位或地图状态；`CourtPanelView` 的朝议批票、廷议要冲、朝议争点和 `CourtProjectDomain` 只服务票拟解释、要冲归属扫读、四线展示、争点表达和项目分组，可执行朝廷项目必须走 `Command.enactCourtProject -> CommandValidator -> CommandExecutor -> EconomyRules`。`HUDView` 的朝报令条/朝报要冲、`BattleObjectivePanelView` 的目标面板与目标定位、`CommandPanelView` 的军令牌/舆图军令/要冲军令/朝议四线、`GeneralCommandPanelView` 的将印军令/帷幄四线/督师要冲、`GeneralProfileView` 的将领名帖、`AgentPanelView` 的军机复盘牌、`EventLogView` 的塘报战记和急务/战役分类、`UnitInspectorView` 的军情牌/要冲牵引、`UnitTooltipView` 的舆图军牌浮签/军位/要冲牵引、`RegionInspectorView` 的州府牌/本州入局/州府四线牵引、`EconomyPanelView` 的府库牌、`DiplomacyPanelView` 的天下急势/天下牵引/天下棋势、`RootGameView` 的舆图天下急势条、要冲分布、本旬先手和舆图点验、`BoardScene.drawFocusedObjective` 的目标定位令牌与胜负线城关连线、`BoardScene.drawPlannedOperations` 的军令计划线、`UnitNode` / `MingFactionFlagBadge` 的势力旗号、`AppContainer.showsSupplyRoutes` 和 `MapDisplayLayer` 图例元数据只控制 UI 展示。`RulerAgent` 仍不是默认主链路。
- `EconomyPanelView` 的“府库四线牵引”“经世策眼”“民食灾荒”和 `DiplomacyPanelView` 的“天下牵引”“天下棋势”同样只属于 UI 展示边界：府库四线牵引从既有府库账本和朝议摘要派生可读文本，经世策眼从既有战役目标摘要、府库账本和朝议摘要派生可读文本，民食灾荒从既有库存粮、本旬粮差、治理摘要和缺粮/断粮军伍派生可读风险，天下牵引从既有外交关系、朝议摘要和最近朱批外交摘要派生可读文本，天下棋势从既有战役目标摘要派生要冲分、最急五线和本旬落点；它们都不新增经济、朝廷、外交、胜负、科技或灾荒字段，不写塘报，不触发生产、目标定位、朝廷项目、AI 决策、灾荒事件或任何命令/规则执行。
- `BattleObjectivePanelView` 的“国势四策”同样只属于 UI 展示边界：它从既有目标摘要、朝议摘要、府库账本和 `CampaignLineBrief` 派生可读文本，联读政策、经济、科技、军事四线，不新增经济、朝廷、胜负、科技或军事字段，不写塘报，不触发生产、目标定位、朝廷项目、AI 决策或任何命令/规则执行。
...
`EconomyPanelView` 当前以府库牌展示 `FactionEconomyLedger`，把库存、入账、军粮维护、补员消耗、收支急报、净民力/银两/粮草、府库四线牵引、经世策眼、民食灾荒、军饷民心、募兵筹粮和营造队列放在同一钱粮界面；府库四线牵引只读 `CourtStrategySummary` 与 ledger 派生四线压力、库存和朝议取舍，经世策眼只读 `BattleObjectiveSummary`、`CourtStrategySummary` 与 ledger 派生领先方、要冲分、最急五线、府库余势和本旬主议；民食灾荒只读库存粮、本旬粮差、治理摘要和缺粮/断粮军伍派生民食余势与灾荒风险，不新增灾荒字段或事件效果；军饷民心只读 ledger、当前势力部队补给状态和治理摘要派生压力，不新增真实军饷/士气/民心字段；募兵筹粮行只读库存与成本差额显示可开工、尚缺资源、待本方或观战；它只读 ledger 并提交生产命令，不直接写 `EconomyState`。
...
- `RegionInspectorView` 以州府牌展示当前州府主值、本州入局、州府四线牵引、治理、钱粮城防、战局归属和当前格；“本州入局”只读取现有 `RegionInspectorState`、`RegionNode`、`OccupationState` 与经济产出，按目标、前线、民变行政、粮台、工坊和驿道派生总批，不新增 schema、不触发目标定位、不写塘报、不提交命令。
...
- `EconomyPanelView` 以府库牌展示当前 active faction 的 `FactionEconomyLedger`，读取库存、入账、维护、补员、净收支、粮草/补员压力、府库四线牵引、经世策眼、民食灾荒、内政钱粮治理摘要、军饷民心只读态势、生产成本、生产可用状态和 `ProductionOrder` 进度；府库四线牵引只读 ledger 与 `CourtStrategySummary` 派生政策、经济、科技、军事压力、库存和主议/备议；经世策眼只读 `BattleObjectiveSummary`、`CourtStrategySummary` 与 ledger，派生 objective points 领先方、最急天下五线、府库粮银余势、本旬主议和备议；民食灾荒只读库存粮、本旬粮差、治理摘要和缺粮/断粮军伍，派生民食余势、粮差、不稳州府和断粮军伍提示，不新增持久灾荒字段或事件效果；治理摘要只读 `GovernanceAISummary`，把州府、不稳、民变、行政和最低行政州府前置给玩家；军饷民心只读当前势力未毁 `Division` 的补给状态、银两余势和同一治理摘要，派生军伍、缺粮、军饷和民心压力，不新增持久字段或规则效果；生产行只读库存与成本差额提示可开工或尚缺资源，生产入口仍只调用 `onQueueProduction -> AppContainer.queueProduction -> Command.queueProduction`，不直接写入 `EconomyState`。

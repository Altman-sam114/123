...
- v4.6 地图标识首片中，`BaseTerrain.displayName` 已切为明末中文地形名；`HexNode` 用“城 / 关 / 粮”badge 标识城池、关隘/堡寨和粮台，并把旧主地图 `FORT`、`SUP A/G` 标记改为“关隘”“粮台”；当前又由 `TerrainStyle.terrainMotif` 和 `HexNode.addTerrainMotif` 为平原、林地、山地、丘陵、城池、关隘增加低透明“田 / 林 / 山 / 丘 / 城 / 关”地貌底纹。该变化只影响 SpriteKit 展示，不改地形类型、补给、占领、战区或经济规则。

...
- v4.7 明末 AI doctrine 记录：`md/prompt/v4.0-明末迁移/v4.7_ming_ai_doctrine_record.md`；当前已新增 `ZoneCommanderDoctrine`，让 `TheaterCommanderPool` fallback、`AppContainer` 空将领 registry fallback、显式 `.zoneDirective` 路径和 `MockAICommander` 默认配置按明廷谨慎、清/大顺/大西进取、地方中立自保生成 `ZoneCommanderAgentConfig`，并让 `ZoneCommanderAgent` 按 doctrine 把同态进攻映射为清方突骑破阵/合围、大顺破围、大西流动作战；该片只改变 directive 生成偏置，不新增 AI 管线，不绕过 `ZoneDirective -> WarCommandExecutor -> RuleEngine`。

...
- 当前 `Faction` 已扩展到 `ming/qing/dashun/daxi/localNeutral`，并保留 `germany/allies` legacy fallback；后续敌我判断应继续走 `DiplomacyState`，不得新增 `.opponent` 依赖。
- 当前单位模型源码仍叫 `Division`，但明末初始数据和玩家可见文案已开始转为关宁铁骑、八旗骑营、红衣炮队、流民军老营和地方团练等口径。
- 当前经济模型底层字段仍是 `manpower/industry/supplies`，但 v4.4 首片已把 UI、日志和 AI 摘要显示为民力、银两、粮草，并把 `OccupationState` 解释为民变/行政掌控。
- 当前 UI、SpriteKit 和 legacy fallback 数据仍有部分 `Ardennes V0`、Germany、Allies、Bastogne、Panzer、NATO 符号等残留；默认明末主游戏 UI 已开始清理，但发布级地图、部队、朝廷与天下局势界面仍未完成。
...
12. 代入感必须侧重中华世界局势，而不是只把阿登规则换成中文名；明廷、后金/清、大顺、大西和地方势力的战和、招抚、粮道、民变、朝议和名分都要在界面和 AI 摘要中可见。
13. 政策、经济、科技、军事四条线都要进入版本路线：首版可以轻量，但不能只偏向单一战斗或单一钱粮系统。
...
明末的“科技”不做现代科技树，而是以军械、火器、城防、农政、水利、驿道、屯田、漕运和训练法度表达。首版可以是少量可解释政策/项目，但必须和经济、军事、朝廷 Agent 接起来。

推荐政策线：

- 征饷：短期增加银两，提升民变/降低行政掌控。
- 赈济：消耗银两/粮草，降低民变，缓解灾荒。
- 屯田：中期提高粮草，见效慢。
- 修城：提高城池/关隘防御和围城承受力。
- 整训团练：提高地方守备，野战上限较低。
- 招抚：影响地方中立、流民军或低忠诚将领的转向。

推荐科技/军械线：

- 火器整备：提升火器营和炮队表现，增加银两/火药消耗。
- 红衣炮维护：提升攻城/守城火力，但机动慢、维护高。
- 驿道与粮台：提升补给路径和后方部署效率。
- 水利/农政：降低灾荒冲击，提高粮草恢复。
- 骑兵马政：提升骑兵补员或机动，消耗马匹/银两。

边界：

- 政策/科技不能直接改 `GameState` 权威状态；必须输出结构化 directive，经 validator / executor 后影响经济、补给、城防或单位修正。
- UI 上应有朝廷/政务入口，让玩家能看见政策选择、代价、风险、预计收益和 Agent 争议。
- 科技/政策必须服务明末局势，不做泛化 4X 科技树。
...
- 让玩家在钱粮、州府和天下局势界面里看见民变、行政掌控、诸方战和与主要对手。
...
- 这些政策后置实现时必须同时写清资源代价、民变/行政影响、军事收益和 AI 摘要字段。
...
- 天下局势：外交/国家面板应显示为“天下局势”，重点表达中华世界内明廷、后金/清、大顺、大西、地方势力的战和关系、张力和名义主体。
...
- AI 摘要能看到粮草、军饷和关键缺口。
- 玩家能在天下局势入口看到诸方势力、战和关系和当前主要对手。
- 经济规则不直接改变 hex 控制权。

### v4.5：皇帝、朝议、政策科技、督师和将领 AI Agent 分层
...
- `raiseTax` / 征饷：银两收益、民变风险、行政掌控下降。
- `relief` / 赈济：粮草/银两消耗、民变下降。
- `fortify` / 修城：提高城防或围城承受力。
- `trainMilitia` / 整训团练：提升地方守备。
- `firearmReform` / 火器整备：提升火器/炮队修正，增加维护。
- `grainTransport` / 粮台转运：提升指定方向补给优先级。
...
- 玩家能看到朝议围绕政策、经济、科技、军事的取舍，不只是战术指令列表。
...
- 朝廷：朝议、政策、科技/军械、钱粮和军令必须有统一的奏疏/军机风格，不能只是英文 debug 列表换中文。
- 天下局势：用清晰层级展示明廷、后金/清、大顺、大西、地方中立的战和、张力、名义主体和主要对手。

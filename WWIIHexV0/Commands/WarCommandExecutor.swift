    ...
    private func defensiveDestination(
        ...
        for regionId in preferredRegionIds {
            ...
            for coord in regionHexes {
                let isCurrentCoord = coord == division.coord
                let isReachable = movementRange.contains(coord)
                let isPassable = state.map.tile(at: coord)?.isPassable == true
                let isUnoccupied = state.division(at: coord) == nil

                if !isCurrentCoord && isReachable && isPassable && isUnoccupied {
